...
function invert(bitmapObj, cb) {
  ...
  var startBit, endBit, jump = checkBPP(bitmapObj);
...
function grayscale(bitmapObj, cb) {
  ...
  for (let i = startBit; i < endBit - jump; i += jump) {
    bitmapObj.bytes[i] = Math.floor(bitmapObj.bytes[i] * 0.1140);
    bitmapObj.bytes[i + 1] = Math.floor(bitmapObj.bytes[i + 1] * 0.5870);
    bitmapObj.bytes[i + 2] = Math.floor(bitmapObj.bytes[i + 2] * 0.2989);
  }
  writeBMP(bitmapObj, transformationName, cb);
}
